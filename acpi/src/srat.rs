        ...
        for entry in self.entries() {
            match entry {
                SratEntry::LocalApic(entry) => {
                    ...
                    if !flags.contains(LocalApicAffinityFlags::ENABLED) {
                        continue;
                    }
                    ...
                    let processor_affinity = ProcessorAffinity { local_apic_id, proximity_domain };
                    processor_affinities[processor_count] = processor_affinity;
                    processor_count += 1;
                }
                SratEntry::LocalX2Apic(entry) => {
                    let flags = entry.flags;
                    if !flags.contains(LocalX2ApicAffinityFlags::ENABLED) {
                        continue;
                    }
                ...
                SratEntry::Memory(entry) => {
                    let flags = entry.flags;
                    if !flags.contains(MemoryAffinityFlags::ENABLED) {
                        continue;
                    }
