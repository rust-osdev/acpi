...
use acpi::{
    ...
    PhysicalMapping,
    address::MappedGas,
    aml::{AmlError, Interpreter, namespace::AmlName, object::AmlString},
...
pub fn new_interpreter<T>(handler: T) -> Interpreter<T, Global>
where
    T: Handler + Clone,
{
    ...
    T: Handler,
{
    ...
    method_bytes.extend(pkglength::encode(opcodes.len() as u32 + 5).iter());
    ...
    T: Handler,
{
    ...
    let result = catch_unwind(AssertUnwindSafe(|| -> Result<(), AmlError<Global>> {
        ...
        if let Some(result) =
            interpreter.evaluate_if_present(AmlName::parse_in("\\MAIN", Global).unwrap(), Vec::new_in(Global))?
        {
            ...
            if result_matches(expected_result, &result) {
                ...
                error!("{}", e);
                Err(AmlError::HostError(AmlString::from_str_in(&e, Global)))
            }
        } else {
            Ok(())
        }
